...
import { appLogo, appLogoCompleto_dark, appLogoCompleto_light, appName } from '../constants'
...
@Component({
  ...
  template: `
    @if (fixTheme === null) {
      <a [routerLink]="logoLink" class="logo-dark">
      ...
      <a [routerLink]="logoLink" class="logo-light">
        <span class="d-flex align-items-center gap-1">
      ...
      <a [routerLink]="logoLink" class="logo-light">
        <span class="d-flex align-items-center gap-1">
          <img [src]="fixTheme === 'dark' ? appLogoCompleto_dark : appLogoCompleto_light" alt="Logo" [style.max-width.px]="logoMaxWidth">
          <!-- <span class="logo-text text-white fw-bold fs-xl m-2">{{appName}}</span> -->
        </span>
      </a>
    }
...
export class AppLogo {
  ...
  @Input() fixTheme: 'dark' | 'light' | null = null;
